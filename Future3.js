...
function detectQuotes(text) {
    const quotePattern = /"([^"]*)"/g; // Detects anything inside quotes
    return text.match(quotePattern) || [];
}

// Function to detect fill-in-the-blanks (any text inside quotes that contains "_")
function detectFillInTheBlanks(text) {
    const blankPattern = /"[^"]*_[^"]*"/g; // Detects "_" inside quotes
    return text.match(blankPattern) || [];
}

// Function to log only detected quotes & blanks to FutureSolver.txt
function logToFile(data) {
    fs.appendFile("FutureSolver.txt", data + "\n", (err) => {
...
function createBot() {
    ...
    // Detect and log only quotes & fill-in-the-blanks (chat, system, server messages)
    bot.on("message", async (jsonMsg) => {
        ...
        if (quotes.length > 0) {
            ...
            logToFile("Quotes: " + quotes.join(", "));
        }
        if (blanks.length > 0) {
            console.log("Detected Fill-in-the-blanks:", blanks);
            logToFile("Fill-in-the-blanks: " + blanks.join(", "));
        }
    });
