...
const fs = require("fs");
const Vec3 = require("vec3"); // Ensure Vec3 is imported
...
async function moveCloser(bot, blockPos) {
    return new Promise((resolve, reject) => {
        try {
            ...
            setTimeout(() => reject(new Error("⚠️ Timeout: Unable to reach block")), 5000);
        } catch (err) {
            reject(new Error(`⚠️ Movement error: ${err.message}`));
        }
    });
}
