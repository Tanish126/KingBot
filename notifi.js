...
const fs = require("fs");
const { exec } = require('child_process');  // Import exec for sending notifications
...
// Function to send Termux notification when the bot wins a chat game
function sendNotification() {
    exec('termux-notification --title "Chat Game Won!" --content "Your bot won a chat game!"', (err, stdout, stderr) => {
        if (err) {
...
function createBot() {
    ...
    bot.on("message", async (jsonMsg) => {
        ...
        // Log correct answer from the server and send notification
        if (msg.includes("ᴛʜᴇ ᴄᴏʀʀᴇᴄᴛ ᴀɴsᴡᴇʀ ᴡᴀs")) {
            ...
            if (correctAnswerMatch) {
                ...
                if (lastSentAnswer === correctAnswer) {
                    ...
                    sendNotification();  // Send notification when bot wins
                } else {
